...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    try{
        ...
        $stmt = $conn->prepare("SELECT * FROM transaction WHERE customer_id = ? AND tdate = ?");
        $stmt->bind_param('ss', $cid, $tdate);
        $stmt->execute();
        $res = $stmt->get_result();
        if($bow = $res->fetch_assoc()){
            $error_message='आप ग्राहक की आज की दैनिक राशि पहले ही जमा कर चुके है पुनः जमा न करे  |';
        }else{
            ...
            if($crow=$result->fetch_assoc()){
                $balance = $crow['wallet_balance']+$amount;
                $collect = $row['wallet_balance']+$amount;

                $stmt = $conn->prepare("UPDATE customer SET wallet_balance = ? WHERE customer_id = ?");
                $stmt->bind_param('ds', $balance, $cid);
                $stmt->execute();

                $stmt = $conn->prepare("UPDATE customer SET wallet_balance = ? WHERE customer_id = ?");
                $stmt->bind_param('ds', $collect, $sponser);
                $stmt->execute();

                $stmt = $conn->prepare("INSERT INTO transaction(customer_id,tdate,amount,collected_by,partner_id) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param('ssdss', $cid, $tdate, $amount, $ename, $sponser);
                $stmt->execute();

                $conn->commit();
                $status = '';

                $succes_msg = '<div class="alert alert-success">Transaction Success..!</div>';
                $stmt->close();

            }else{
                 echo "<script>alert('Customer Id is Invalid..!')</script>";
                 $status = ''; 
            }
        }
    }catch(Exception $e){
                ...
                <section class="modal-container-body rtf">
                    <form id="loginForm" method="POST">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="customer" required name="sponser" readonly value="<?php echo $row['customer_id']; ?>">
                        </div>
                        <div class="form-group">
                            <label for="font-awesome">Customer ID</label>
                            <input type="text" class="form-control" id="customer" placeholder="Enter Customer ID" required name="cust_id">
                        </div>
                        <div class="form-group">
                            <label for="amount">Collected Amount</label>
                            <input type="number" class="form-control" id="amount" placeholder="Enter Amount" required name="amount" maxlength="4" value="5" readonly>
                        </div>
                        
                        <div class="form-group">
                            <label for="epin">Employee PIN</label>
                            <div class="input-group">
                                <input type="number" class="form-control" id="password" placeholder="PIN" required name="epin" maxlength="6">
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary btn-block" <?php echo $status; ?>>Collect</button>
                    </form><br>
                    <?php
                        echo $succes_msg;
                    ?>
