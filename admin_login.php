...
<body>
    ...
    <div class="table-responsive border-primary">
        <div class="alert alert-success">Transaction of : <?php echo date('l jS \of F Y'); ?></div>
        <table class="table">
        <thead class="primary">
            <td>No.</td>
            <td>ID</td>
    ...
    <?php 
    $tdate = "".date('d-m-Y');
    $stmt = $conn->prepare("SELECT * FROM transaction WHERE tdate = ?");
    ...
    if($res->num_rows>0){
        ...
        echo '<td colspan="5" class="text-danger text-center">No Records Found ('.date("l jS \of F Y").')</td>';
    }
        ?>
    </table>
    </div>
